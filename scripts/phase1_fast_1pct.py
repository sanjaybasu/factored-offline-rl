"""
Phase 1: Fast Data Preparation (1% Sample)
===========================================

Ultra-fast version using only 1% of members for rapid training validation.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/Users/sanjaybasu/waymark-local/notebooks/factored_rl/phase1_1pct.log'),
...
def main():
    logger.info("="*80)
    logger.info("PHASE 1: ULTRA-FAST DATA PREPARATION (1% SAMPLE)")
    logger.info("="*80)
    ...
    output_dir = Path('/Users/sanjaybasu/waymark-local/notebooks/factored_rl/data_1pct')
    ...
    sample_size = max(int(len(all_members) * 0.01), 1000)  # At least 1000 members
    ...
    logger.info(f"  Sampled: {len(df_sample):,} observations from {len(sampled_members):,} members (1%)")
    ...
    for member_id in sampled_members:
        ...
        for i in range(len(member_df) - seq_length + 1):
            ...
            last_row = seq.iloc[-1]
            seq_data['action'] = last_row.get('action_tuple', 0)
    ...
    logger.info("="*80)
    logger.info("✓ PHASE 1 COMPLETE (1% SAMPLE)")
    logger.info("="*80)
