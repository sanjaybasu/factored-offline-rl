...
class RewardShaper:
    ...
    def compute_primary_reward(self, ed_visits: int, hospitalizations: int) -> float:
        ...
        Rationale: Hospitalizations are weighted 2x because they are:
        - More costly ($10K vs $2K)
        - More severe outcomes
        - Stronger signal of care management failure
...
def main():
    ...
    # Load trajectory data (placeholder)
    data_path = Path("/Users/sanjaybasu/waymark-local/notebooks/factored_rl/data")
    trajectory_df = pd.read_parquet(data_path / "enhanced_trajectories_daily.parquet")
    ...
    # Save shaped rewards
    output_path = data_path / "trajectories_with_shaped_rewards.parquet"
    ...
    plt.tight_layout()
    plt.savefig(data_path.parent / 'figures' / 'reward_distributions.png', dpi=300)
