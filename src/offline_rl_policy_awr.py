...
class ActionEmbeddingNetwork(nn.Module):
    ...
    Avoids a 1080-way softmax; scales with embedding dim, not action count.
    """
    ...
    def forward(self, states: torch.Tensor, action_embeddings: torch.Tensor) -> torch.Tensor:
        ...
        if action_embeddings.dim() == 2:
            ...
            x = self.fc3(x)
            # Bound output to prevent divergence
...
def main():
    ...
    print("Note: Requires factored action data; using synthetic demo below.")
